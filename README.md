...
```
olimp_win/
├── EduBattle.exe          # Основной исполняемый файл
...
Или, если переменная установлена через системные настройки — просто запустите **`EduBattle.exe`** двойным кликом.
...
Также можно использовать **встроенный редактор базы данных** (`db_editor/run.py` на Windows / `db_editor` на Linux) из комплекта поставки.
